...
| 2026-07-09 | LiveKit signaling TLS-terminated via nginx `/livekit` proxy + frontend scheme auto-upgrade + `LIVEKIT_PUBLIC_URL` env override | On an HTTPS deployment the browser blocked the LiveKit `ws://` signaling URL as mixed content (same class of bug as the game WS `wss://` fix). Three-part fix: (1) `AvClient.connect()` upgrades `ws://`→`wss://` and `http://`→`https://` when `window.location.protocol === "https:"`, mirroring the GameScene WS fix — the LiveKit SDK derives both the signaling WS and the `/rtc/v1/validate` fetch from the URL scheme, so one upgrade fixes both; (2) in-container `nginx.conf` now proxies `/livekit/` → `http://livekit:7880/` (trailing-slash prefix strip) in both server blocks, so LiveKit signaling is TLS-terminated same-origin through the existing nginx cert; (3) `docker-compose.yml` `LIVEKIT_PUBLIC_URL` changed from a hardcoded `ws://${PUBLIC_HOST}:7880` to `${LIVEKIT_PUBLIC_URL:-ws://${PUBLIC_HOST}:7880}` (nested interpolation, supported in Compose v2) so a `.env` override actually wins — previously the compose file assigned a literal string and ignored the env var, so the documented "set `LIVEKIT_PUBLIC_URL` in `.env`" workflow silently did nothing. Deployers set `LIVEKIT_PUBLIC_URL=ws://<domain>/livekit` (auto-upgraded to `wss://`). A ready-to-copy `docker/dist/example.nginx.conf` is provided for host nginx. WebRTC media (UDP 50000-50020) still needs direct reachability — signaling over `wss://` is necessary but not sufficient for full A/V. |
