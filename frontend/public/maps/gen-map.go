...
func main() {
	...
	// --- Generate map data ---
	// Tile GIDs: firstgid+0=grass, firstgid+1=wall, firstgid+2=floor, firstgid+3=path
	// Layer 0 (ground): grass everywhere, floor in center room
	ground := make([][]int, mapH)
	for y := range mapH {
		ground[y] = make([]int, mapW)
		for x := range mapW {
			ground[y][x] = firstgid + 0 // grass
		}
	}
	// Inner room floor (tiles 4..16)
	for y := 4; y <= 16; y++ {
		for x := 4; x <= 16; x++ {
			ground[y][x] = firstgid + 2 // floor
		}
	}
	...
	// Map border
	for i := range mapW {
		walls[0][i] = firstgid + 1
		walls[mapH-1][i] = firstgid + 1
	}
	for i := range mapH {
		walls[i][0] = firstgid + 1
		walls[i][mapW-1] = firstgid + 1
	}
	// Room walls (around the floor area)
	for i := 4; i <= 16; i++ {
		walls[4][i] = firstgid + 1
		walls[16][i] = firstgid + 1
		walls[i][4] = firstgid + 1
		walls[i][16] = firstgid + 1
	}
	...
	tiledMap := map[string]any{
		...
		"tilesets": []map[string]any{
			{
				// Embedded tileset — Phaser 4 does not support external tileset
				// references (source field). Inline all tileset properties here.
				"columns":     tileCount,
				"firstgid":    firstgid,
				"image":       "tileset.png",
